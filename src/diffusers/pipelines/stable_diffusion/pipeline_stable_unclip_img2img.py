...
class StableUnCLIPImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        blend_img: Union[torch.FloatTensor, PIL.Image.Image] = None,
        blend_weight=0.5
    ):
        r"""
        ...
        if blend_img != None: # 插值
            blend_img = self._encode_image(
                image=blend_img,
                device=device,
                batch_size=batch_size,
                num_images_per_prompt=num_images_per_prompt,
            ...
            image_embeds1, image_embeds2 = image_embeds.chunk(2)
            image_embeds2 = slerp(blend_weight, image_embeds2, blend_img)

            image_embeds = torch.cat([image_embeds1, image_embeds2])
